...
# Install linux-ms {{{
COPY --chown=1000:1000 . /opt/fmc_repository/linux-ms
RUN install_repo_deps.sh /opt/fmc_repository/linux-ms/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/linux-ms/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
